...
| # | 카테고리 | 참조 파일 | 대표 키워드 |
|---|---------|----------|------------|
| 1 | 외부 커뮤니케이션 | [external-comm.md](references/external-comm.md) | 이메일, 위기 대응, 보도자료, 뉴스레터, 사례 연구, SNS, 블로그, 출시 메시지, FAQ, CS |
...
| 5 | 분석·법무 | [analysis-legal.md](references/analysis-legal.md) | 계약서, 법률 의견서, 소장·답변서·준비서면, 감사·세무 의견서, 실사·내부통제. 결론은 메모 톤 |
...
작업 영역마다 자연스러운 종결 체가 다릅니다. 한 산출물 안에서 두 종결 체가 섞이면 위반입니다.

| 작업 영역 | 종결 체 |
|-----------|---------|
| 분석·보고·평론(보고서, 메모, 평가, 이력서 본문) | `~이다` 체 |
| PPT 헤드라인·액션 타이틀 | `~함·~임` 체 (명사형 종결) |
| 뉴스레터 | `~입니다` 체 |
| 외부 비즈니스 이메일·CS 응답·사과문 | `~합니다·~드립니다` 체 |
| 보도자료·신문 칼럼·법률 의견서·계약서 검토·감사 의견서 | `~다·~이다` 체 |
| SNS 포스트·블로그(개인 톤)·시·단편 소설 | 작성자 톤 우선 |

분석·법무 카테고리의 결론 섹션은 위 종결 체에 메모 톤이 더해집니다. 단정형 종결어미 + 행동 동사(권고·진행·보류·정정 등) + 주체 명시(이사회·법무팀장·재무팀장 등)가 한 줄에 들어갑니다.

---

## em dash 정책

한국어 산출물에서 em dash(`—`, `–`)는 영문 호흡이라 어색합니다. 본 스킬이 생성한 system instruction을 적용한 모든 산출물은 본문에 em dash 0개를 강제합니다. 한 개라도 발견되면 자동 fail이며, 마침표·쉼표·괄호·콜론·새 문장 중 하나로 즉시 교체합니다. 자체 점검 복구 우선순위 표의 1번 항목입니다.
